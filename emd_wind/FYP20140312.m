...
%     myIMFstatistical_significance_test(CEEMDAN_IMF(trnIdx,:)', [0.05, 0.95]);
    ...
    %% CEEMD-BPNN
    % preallocation
    CEEMD_ANN_IMF_pred=cell(1,size(CEEMD_IMF,2));
    for i=1:size(CEEMD_IMF, 2)
        % preallocation
        tmp_ceemd_ann_pred=zeros(size(tst_labels,1), max(HORIZON_array));
        
        for HORIZON=HORIZON_array
            [ tmp_ceemd_ann_pred(:,HORIZON), tmp_ceemdann_cv_mse(:,HORIZON) ] = ...
        ...
        ceemdann_cv_mse{i}=tmp_ceemdann_cv_mse;
    end
    ...
    CEEMD_IMF_pred=cell(1,size(CEEMD_IMF,2));
    for i=1:size(CEEMD_IMF, 2)
    ...
    CEEMD_CV_ACC{idx}=CEEMD_IMF_CV_accuracy;
    CEEMD_CV_MSE{idx}=ceemdann_cv_mse;
    CEEMD_B_P{idx}=CEEMD_IMF_best_param;
    CEEMDAN_CV_ACC{idx}=CEEMDAN_IMF_CV_accuracy;
    CEEMDAN_CV_MSE{idx}=ceemdanann_cv_mse;
    CEEMDAN_B_P{idx}=CEEMDAN_IMF_best_param;
    pred{idx}=[persistent_pred, bpnn_pred, svr_pred, emdann_pred, emd_pred, eemd_pred, eemdann_pred, ceemdann_pred, ceemd_pred, ceemdanann_pred, ceemdan_pred];
end
save('result_FYP_load_20140424.mat', 'result', 'TST_LB', 'pred','delays','CV_ACC', 'B_P', 'IMF_CV_ACC', 'IMF_B_P', 'EEMD_CV_ACC', 'CEEMD_CV_ACC', 'EEMD_B_P', 'CEEMD_B_P', 'CEEMDAN_CV_ACC', 'CEEMDAN_B_P', 'IMF_CV_MSE', 'EEMD_CV_MSE', 'CEEMD_CV_MSE', 'CEEMDAN_CV_MSE' );
